...
class NepseDashboard(App):
    ...
    def _attach_tms_live_source(self) -> None:
        """Create a TMSSource (httpx + WS) and wire it into self.md.

        Runs after Playwright login succeeds. Silent on failure so a stale
        session.json never blocks the TUI — the header falls back to DB quotes.
        """
        try:
            from backend.quant_pro.tms_source import TMSSource
            src = TMSSource.try_create()
            if src is None:
                return
            src.start_background()
            self._tms_live_src = src
            self.md.tms = src
            try:
                self.md.tms_balance = src.balance()
                self.md.tms_indices = src.indices()
            except Exception:
                pass
        except Exception:
            pass
